		...
		for (y = 31; y<=39; y++){
			...
			for (x = 31; x<=39; x++){
				...
				if(z>65 && z<75){
					...
					//img.onload = function() {
					ctx.drawImage(img, x, y, canvas.width, canvas.height);
					gCtx.drawImage(canvas, x, y, canvas.width, canvas.height);
					//};
					...
					if(ys==y && xs==x && mode!=3 && mode!=6){	
						if(mode==1||mode==4){
							...
							if(j[x][y]==0){img.src = '..//imgs//s.png';};//img depl
							if(j[x][y]==1){};//img depl
							if(j[x][y]==2){};//img depl
						};
						if(mode==2||mode==5){
						...
						if(((ys==ysd&&xs==xsd)||(mode!=2&&mode!=5))||(x!=xsd||y!=ysd)){
							if(j[x][y]==0){img.src = "..\imgs\c.png"};//case vide
							if(j[x][y]==1){img.src = "..\imgs\a.png"};//j1
							if(j[x][y]==2){img.src = "..\imgs\b.png"};//j2
						}else{
							if(j[x][y]==0){};//img depl
							if(j[x][y]==1){};//img depl
							if(j[x][y]==2){};//img depl
