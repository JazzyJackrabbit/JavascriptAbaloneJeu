jQuery(document).ready(function($){
	...
	function Frames(){
		...
		if(mode==8||mode==9){// si c'est la fin de la partie
			...
			for(afftot = 0; afftot < 130;afftot++){affichage()}; 	//affiche les cases et les billes du jeu
				...
				if(zaff>65 && zaff<75){
					...
					if(ys==yaff && xs==xaff && mode!=3 && mode!=6){
						if(mode==1||mode==4){
							...
							if(j[xaff][yaff]==2){img = imgp;
							testk = 1;
							};//img depl
						};
						if(mode==2||mode==5){
							//affiche selecteur mode direction
							if(j[xaff][yaff]==0){img = imgd;};//pos
							if(j[xaff][yaff]==1){img = imgd;
							testk = 1;
							};//pos
							if(j[xaff][yaff]==2){img = imgd;
							testk = 1;
							};//pos
						...
						if(((ys==ysd&&xs==xsd)||(mode!=2&&mode!=5))||(x!=xsd||y!=ysd)){
							if(j[xaff][yaff]==0){img = imgc;};//case vide
							if(j[xaff][yaff]==1){img = imga;
							testk = 1;
							};//j1
							if(j[xaff][yaff]==2){img = imgb;
							testk = 1;
							};//j2
						}else{
							if(j[xaff][yaff]==0){img = imgp;};//img depl
							if(j[xaff][yaff]==1){img = imgp;};//img depl
							if(j[xaff][yaff]==2){img = imgp;};//img depl
						};
					};
					...
					//img.onload = function() {
						...
						if(yaff>=31&&yaff<=39&&xaff>=31&&xaff<=39){

							ctx.clearRect ((tX)*taille+3, (tY+tX/2)*(taille*addtailley)-110,48,48);
							ctx.drawImage(img, (tX)*taille+3, (tY+tX/2)*(taille*addtailley)-110,48,48);
							
							if (testk==1){ctx.drawImage(imge, (tX)*taille+7, (tY+tX/2)*(taille*addtailley)-111,35,35);}
